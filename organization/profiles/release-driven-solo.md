...
u-sekai の current solo / small-project release-driven development で使用する Operating Model を定義します。
...
## u-sekai explicit decisions

- Project-authored documentation, GitHub Issues, Pull Requests, and review discussion are English-primary per `docs/adr/ADR-0002-language-policy-override.md`.
- Upstream-authored Agent Skills remain verbatim Japanese copies so policy semantics stay aligned with the pinned upstream source.
- These language decisions are project-specific deviations; the remaining current defaults apply unless a newer u-sekai ADR explicitly replaces them.

## Upstream source

Reconciled from `rebuildup/project-init@release-0-3-0` (`57fb4a2e5abe6f52e4fd9cb2cb88234496e47d4b`).
